...
PCI DSS recommends that the S3 bucket policy, or access control list (ACL), applied to the S3 bucket that CloudTrail 
...
This rule is covered by the [cloudtrail-s3-bucket-should-not-be-publicly-accessible](https://github.com/hashicorp/policy-library-pcidss-policy-set-for-aws-terraform/blob/main/policies/cloudtrail/cloudtrail-s3-bucket-should-not-be-publicly-accessible.sentinel) policy.

## Policy Results (Pass)
```bash
trace: 

    Pass - cloudtrail-s3-bucket-should-not-be-publicly-accessible.sentinel
    ...
    This result means that all resources have passed the policy check for the policy cloudtrail-s3-bucket-should-not-be-publicly-accessible.
...
trace:
    Fail - cloudtrail-s3-bucket-should-not-be-publicly-accessible.sentinel
    ...
    This result means that not all resources passed the policy check and the protected behavior is not allowed for the policy cloudtrail-s3-bucket-should-not-be-publicly-accessible.
...
=======
    Rule "main"
>>>>>>> 1236c0ad1b5dae9522b57fb9dde7918d2b41a150
      Value:
        false
```
